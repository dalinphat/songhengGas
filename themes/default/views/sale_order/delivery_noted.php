            ...
            <tr>
                <td width="57%"  style="padding: 3px;">
                    <table width="100%" style="height: 100%" >
                        <tr>
                            <td>
                                <span class="com_name"><?= $biller->company ?></span>
                            </td>
                        </tr>
                        <tr>
                            <td>
                               ...
                               <div class=" " style="font-size: 12px; border: 1px solid black;border-radius: 5px !important; padding: 6px;width: 100%;height: 110px; ">
                                   <table >
                                       ...
                                       <tr>
                                           ...
                                           <td>:</td>
                                           <td><?= $invs->saleman_first.' '.$invs->saleman_last ?></td>
                                       </tr>
...
<!--                <th> នាយឃ្លាំង<br/>--><?//= strtoupper(lang('wh counter')) ?><!--</th>-->
            ...
            $i=0;
            $qty_so=[];
            if($soQty=$this->db->query("select erp_sale_order.id from erp_sale_order where erp_sale_order.reference_no='".$invs->so_no."' " )->result()){
                foreach ($soQty as $sq){
                    if($sq->id){
                        $so_id=$sq->id;
                        if($Qtyso=$this->db->query("select erp_sale_order_items.quantity from erp_sale_order_items where erp_sale_order_items.sale_order_id='".$so_id."' " )->result()){
                            foreach ($Qtyso as $sqty){
                                $qty_so[$i]=$sqty->quantity;
                                //echo $i.'--'.$qty_so[$i].'<br>';
                                $i++;
                            }
                        }
            ...
            foreach ($rows as $row) {
                ...
                <tr class="border tb_str ">
                    ...
                    <td style="border-top:none !important;border-bottom:none !important; text-align: center;">
                        <?=$this->erp->formatQuantity($qty_so[$ii]);?>
                    </td>
                    ...
                    <td style="border-top:none !important;border-bottom:none !important; text-align: center;">
                       <?= $this->erp->formatQuantity(($qty_so[$ii]-$row->quantity))?>
                    </td>
                    <td style="border-top:none !important;border-bottom:none !important; text-align: center;">
...
//                      $this->erp->print_arrays($customer);
            ?>
            <tr class="border">
                <td colspan="8" style="font-size: 12px"><p>សម្គាល់ / Message :</p><p><?php echo $customer->invoice_footer ?></p></td>
            </tr>
            </tbody>
        </table>
        <br>
