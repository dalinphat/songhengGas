                        ...
                        foreach($dataEquity->result() as $rowequity){
                            ...
                            for($i = 1; $i <= count($new_billers); $i++){
                                ...
                                if(($index+1)==1){
                                    ...
                                    echo '<td class="right"><a href="<?= site_url(\'reports/ledger/?w=1\'.$from_st.$to_st.\'&account=\'.$rowequity->account_code) ?>">' .number_format(abs($totalBeforeAyearRows->amount),2). '</a></td>';
                                }
    ...
    $(document).ready(function () {
        $("#biller-filter").on('click', function(event){
            ...
            if(hasCheck == true){
                ...
                var url = "<?php echo site_url('reports/balance_sheet/'.$from_date.'/'.$end.'/0/0') ?>" + '/' + encodeURIComponent(billers);
                window.location.href = "<?=site_url('reports/balance_sheet/'. $from_date .'/'.$end.'/0/0/')?>" + '/' + encodedName;
            }
        ...
        $('#pdf').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('reports/balance_sheet_details/'. $from_date .'/'.$end.'/pdf/0/'.$excel_biller_id)?>";
            return false;
        });
        $('#xls').click(function (event) {
            event.preventDefault();
            window.location.href = "<?=site_url('reports/balance_sheet_details/'. $from_date .'/'.$end.'/0/xls/'.$excel_biller_id)?>";
            return false;
        });
