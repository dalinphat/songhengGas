...
if($_POST['reference_no']){
    ...
    $reference_no1=str_replace(' ','aa',$reference_no);
}
//echo '<h1 style="background: red;width: auto">ssssss'.@$reference_no1.'</h1>';
if($_POST['start_date'])
{
    $start=$this->erp->fld($_POST['start_date']);
}
if($_POST['end_date']){
    $end=$this->erp->fld($_POST['end_date']);
}
if($_POST['biller']){
    $biller_ii=$this->erp->fld($_POST['biller']);
}
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-users"></i><?= lang('Purchase_by_item_detail'); ?></h2>
...
//                $this->erp->print_arrays($purchase_id);
                ...
                foreach ($purchase_id as $cu){
                    ...
                    $product_detail = $this->reports_model->getPurchaseDataAll_r($cu->product_code, $reference_no1,$start,$end,$biller_ii);
                    ...
                    foreach ($product_detail as $aa) {
                        ...
                        <tr >
                            ...
                            <td style=" padding: 1px !important;"  class="text-center" >
                                <a href="<?= site_url('purchases/modal_view/' . $aa->pu_id) ?>" data-toggle="modal" data-target="#myModal2" class="tip btn btn-primary" style="padding: 2px 10px" title="View Invoice">

                                    <span class="hidden-sm hidden-xs">View Purchase</span>
                                </a>
                            </td>
                        </tr>
