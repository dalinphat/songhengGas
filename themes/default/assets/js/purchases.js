...
// Order discount calcuation
    ...
    $(document).on('click', '.edit', function (){
        ...
        var product_option 	= row.children().children('.roption').val(),
            ...
            warehouse_id 	= row.children().children('.piwarehouse').val(),
        ...
        $('#piwarehouse').select2('val', warehouse_id);
	...
	*/
    ...
    $(document).on('change', '#piwarehouse', function () {
        var warehouse_id = $(this).val();
        $('#pi_warehouse').val(warehouse_id);
    });
    ...
    $(document).on('click', '#editItem', function () {
        ...
        poitems[item_id].row.pi_warehouse 	    = $('#pi_warehouse').val();
        __setItem('poitems', JSON.stringify(poitems));
        $('#prModal').modal('hide');
        loadItems();
    ...
    if (__getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            var product_id 		= item.row.id,
                ...
                item_warehouse	= item.row.pi_warehouse?item.row.pi_warehouse:__getItem('powarehouse');
            ...
            if (ds.indexOf("%") !== -1) {
                ...
                if (!isNaN(pds[0])) {
                    ...
                    if (qty_received) {
                        item_discount = parseFloat(ds/qty_received);
                    }
                    else {
            ...
            /* Unit of Measure */
           /* tr_html += '<td class="text-center">hel</td>';*/

            /* Stock In Hand */
            tr_html += '<td class="text-right"><input class="form-control input-sm text-right rstock" name="rstock[]" type="hidden" id="stock_' + stock_in_hand + '" value="' + stock_in_hand + '"><input class="rstock" name="rstock[]" type="hidden" value="' + stock_in_hand + '"><input class="rstock" name="rstock[]" type="hidden" value="' + stock_in_hand + '"><input class="piwarehouse" name="piwarehouse[]" type="hidden" value="'+ item_warehouse +'"><span class="text-right scost" id="sstock_' + row_no + '">' + stock_in_hand + '</span></td>';
